...
determine how many of these pairs overlap *at all* - i.e. pairs that share even a single value.
...
func main() {
	...
	for s.Scan() {
		...
		//2 ranges overlap if they share ANY value between them
		if (r.e1S >= r.e2S && r.e1S <= r.e2F) || (r.e2S >= r.e1S && r.e2S <= r.e1F) {
			fmt.Println(r)
			anyOverlaps++
		}
	}
