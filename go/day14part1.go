...
How many units of sand can come to rest before the remainder "falls off" the map to descend
forever?
...
func buildMap(fromFile string, sandPoint coord) ([][]texture, coord) {
	...
	//"Grow" the map by 1 across X
	fmt.Printf("X range was: %d - %d\n", mDim[0].x, mDim[1].x)
	mDim[0] = coord{x: mDim[0].x - 1, y: mDim[0].y + 1}
	mDim[1] = coord{x: mDim[1].x + 1, y: mDim[1].y + 1}
...
func addSand(cavemap [][]texture, sandpoint coord) ([][]texture, bool) {
	...
	sP := sandpoint
	for !sandCameToRest && sP.y < len(cavemap)-1 {
		...
		if cavemap[sP.y+1][sP.x] == v {
			...
			sP.y = sP.y + 1
			sP.x = sP.x + 1
		} else { //No, it's stuck. Add it to the map and setup return
			cavemap[sP.y][sP.x] = s
			sandCameToRest = true
		}
	}
	return cavemap, sandCameToRest
}
...
func main() {
	...
	sandCounter := 0
	for restingSand {
		caveMap, restingSand = addSand(caveMap, entryPoint)
		if restingSand {
			sandCounter++
		}
